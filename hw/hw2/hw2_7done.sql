...
SELECT T.ClassId
  ...
  AS EmpsRegisteredCompleted
  ,C.Name AS ClassName
FROM Class C
LEFT JOIN Transcript T
ON C.ClassId = T.ClassId
GROUP BY ClassId
ORDER BY EmpsRegisteredCompleted DESC ;
...
/*
,COUNT(DISTINCT(Transcript.ClassId)) AS ClassCount
  ,COUNT(DISTINCT(Transcript.EmployeeId)) AS EmployeeCount
